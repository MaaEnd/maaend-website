...
const OptimizedArchitectureModel = ({ isDark }: { isDark: boolean }) => {
    ...
    const geometry = useMemo(() => {
        ...
        geometry.setAttribute('position', new THREE.BufferAttribute(new Float32Array(temp), 3));
        return geometry;
...
const OptimizedHoloField = ({ isDark }: { isDark: boolean }) => {
    ...
    return (
        <group ref={ref}>
            <mesh position={[0, 0.5, 0]} geometry={geometries.outer} material={materials.outer} />
            <mesh position={[0, 2, 0]} geometry={geometries.inner} material={materials.inner} />
        </group>
    );
};
...
export default function InteractiveModelOptimized() {
    ...
    return (
        <div ref={containerRef} className="group relative h-full w-full cursor-crosshair">
            <Canvas
            ...
            <div className="pointer-events-none absolute inset-0">
                ...
                <div className="absolute top-1/2 left-1/2 h-[60%] w-[60%] -translate-x-1/2 -translate-y-1/2 rounded-full border border-[#00aecb]/10 dark:border-[#00F0FF]/10 animate-spin"
